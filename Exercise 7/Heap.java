public class Heap {
    ...
    public void fixHeap(int i){
        ...
        if(m < length){
            int right = m + 1;
            if(right < length && node[right].occurrences < node[m].occurrences){
                m = right;
            }
            if(node[m].occurrences < node[i].occurrences){
    ...
    public void prioUp(int i, int p){
        int f;
        node[i].occurrences += p;
        while(i > 0 && node[i].occurrences < node[f=over(i)].occurrences){
